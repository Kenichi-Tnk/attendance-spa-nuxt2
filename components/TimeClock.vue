<template>
  <div class="time-clock">
    ...
    <div v-if="workStartTime" class="time-clock__work-info">
      <div class="time-clock__work-start">出勤時刻: {{ workStartTime }}</div>
...
export default {
  ...
  computed: {
    attendanceStatus() {
      return this.$store.state.attendance.currentStatus;
    },
    workStartTime() {
    ...
    canBreakEnd() {
      // 休憩中のみ休憩終了可能
      return (
        this.attendanceStatus === "on_break" ||
        this.attendanceStatus === "on-break"
      );
    },
  },
  mounted() {
